...
namespace TOLEYIS.Controllers
{
    public class InfoController : Controller
    {
        ...
        {
            var enf = db.Enflasyon.Where(x => x.Sil != true).ToList();
            return View(enf);
        }
        ...
        public ActionResult TefeTufe(int? Aylar, int? Yillar, int sayfa = 1)
        {
            ViewBag.dosyayolu = manager.GetAdminIPAndPort();
        ...
        public ActionResult AclikYoksulluk(int? Aylar, int? Yillar, int sayfa = 1)
        {
            ...
            if (Aylar != null && Yillar != null)
            {
                var Aclik = db.AclikDosya.Where(x => x.Aclik.Id == x.AclikID && x.Aclik.Ayid == Aylar && x.Aclik.Yilid == Yillar && x.Aclik.Sil != true).ToList().ToPagedList(sayfa, 8);
                return View(Aclik);
            }
            else if (Aylar == null && Yillar != null)
            {
                var Aclik = db.AclikDosya.Where(x => x.Aclik.Id == x.AclikID && x.Aclik.Yilid == Yillar && x.Aclik.Sil != true).ToList().ToPagedList(sayfa, 8);
                return View(Aclik);
            }
            else if (Aylar != null && Yillar == null)
            {
                var Aclik = db.AclikDosya.Where(x => x.Aclik.Id == x.AclikID && x.Aclik.Ayid == Aylar && x.Aclik.Sil != true).ToList().ToPagedList(sayfa, 8);
                return View(Aclik);
            }
            else
            {
                var Aclik = db.AclikDosya.Where(x => x.Aclik.Sil != true).ToList().ToPagedList(sayfa, 8);
                return View(Aclik);
            }
        }
    }
