...
namespace TOLEYIS.Controllers
{
    ...
    public class AnaSayfaController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            model.Duyurus = db.Haber.Where(x => x.Sil != true && x.TurID == 2).OrderByDescending(x => x.Id).Take(10).ToList();
            ...
            ViewData["slidee1"] = haber.OrderByDescending(x => x.Tarih);
            ViewData["slidee2"] = haber.OrderByDescending(x => x.Tarih).Take(5);
        ...
        public JsonResult KayitKontrol(SiteUyelik p1)
        {
            ...
            if (tck.Count == 0)
            {
                ...
                db.SaveChanges();
                return Json(new { success = true, responseText = "KAYIT BAŞARIYLA GERÇEKLEŞTİRİLMİŞTİR." }, JsonRequestBehavior.AllowGet);
            }
            else
            {
                return Json(new { success = true, responseText = "BU TC NUMARASI İLE KAYITLI ÜYE ZATEN MEVCUT !" }, JsonRequestBehavior.AllowGet);
            }
